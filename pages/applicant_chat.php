...
<body>
        ...
        ");
            ...
            ?>
                    ...
                    <div class="col-lg-4 left-tab">
                        ...
                        <ul class="nav nav-pills flex-column m-0 p-0">
                            ...
                            for ($i = 0; $i < count($existing_conversation); $i++) {
                                ...
                                $other_user = select("vw_admin_basics", "`id` = '$existing_conversation[$i]'");
                                if (mysqli_num_rows($other_user) > 0) {
                                    while ($row = mysqli_fetch_assoc($other_user)) {

                                        echo "<li class='nav-item '>
                                                <a class='nav-link conversation-tab' id='conversation_tab_" . $row['id'] . "' data-bs-toggle='pill' href='#tab_$i'>
                                                "  . $row['full_name'] . " - " . ucwords($row['department']) . "
                                                </a>
                                        </li>";
                                    }
                                }
                                            ...
                                            while ($row = mysqli_fetch_assoc($other_user)) {
                                                ...
                                                echo " <div class='tab-pane fade flex_container' id='tab_$i' style='position:relative;'>
                                                        <div class='row text-start'>
                                                            <h3>" . $row['full_name'] . " - " . ucwords($row['department']) . "</h3>
                                                        </div>
               ...
               ";
                                                ...
                                                echo '
            <div class="text-center write-cont row w-100" id = "" style = "height:50px; position:absolute; margin-top: 17px">
    <div class="col">
    ...
    <div class="col-lg-2 col-12">
        <input type="button" value="Send" class="send-btn" onclick="send_message(\'message_' . $existing_conversation[$i] . '\')">
                ...
                <div class="row text-center m-auto p-5 pb-2">
                <p>Hello there! Thank you for using our services. If you need any help, feel free to 
                ask questions to any of our administrators from their respective departments.</p>
    
                <p>Who would you like to talk to?</p>
                </div>';


                $admins = select("vw_admin_basics", "");


                if (mysqli_num_rows($admins) > 0) {

                    echo ' <div class="row m-auto" style = "padding:0px 4rem 0px 30px">';
        ...
        <a href='#' class='button admin-card' id ='admin_" . $row['id'] . "' data-admin-id = '" . $row['id'] . "'>

        <img src='../php/get_image.php?id=" . $row['id'] . "' class = 'img img-responsive my-img'>
        <p class = 'mt-4 mb-0'><b>" . ucwords($row['full_name']) . "</b></p> 
        <p class = ''>" . ucwords($row['department']) . " Department</p> 
        </a>
        ...
        <!-- Modal -->
        <div class="modal modal-lg fade" id="modal_new_chat" tabindex="-1" aria-labelledby="modal_new_chat_label" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content" >
                    <div class="modal-header">
                        <h1 class="modal-title fs-5" id="modal_new_chat_label">New Conversation</h1>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body text-center" >

                        <?php
                        $admins = full_query("SELECT *
                        FROM vw_admin_basics
                        WHERE id NOT IN (
                            SELECT DISTINCT `from` FROM (
                                SELECT `from` FROM chat_messages
                                UNION
                                SELECT `to` FROM chat_messages
                            ) AS chat_users
                        );
...
function loadMessages(containerId) {
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        let admin_cards = document.querySelectorAll('a.admin-card');

        admin_cards.forEach(inp => {
            inp.addEventListener('click', function handleClick(event) {
                ...
                insert_ajax("chat_messages", "`id`,`from`,`to`,`message`", "UUID(),'" + admin_id + "'," +
                    "'<?php echo $_SESSION['user_id'] ?>','Hello there <?php echo $fullname ?>. We are here to assist you. Please feel free to ask us questions.'")
